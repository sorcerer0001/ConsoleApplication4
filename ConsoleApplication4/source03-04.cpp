...
int CWorld::f_Monitor(CM cm){//不使用了
	//int res = cm.f_ProSmu(ia_ProCos);
	if (cm.b_Flag){
		//cout << setw(3) << setfill('0') << i_Hou << " " << cm.c_Nam << " headquarter stops making warriors" << endl;
		//cout << setw(3) << setfill('0') << i_Hou << " "
		cout<< cm.c_Nam << " " << sa_SmuNam[cm.ia_ProOrd[cm.i_ProKey]] << " " <<cm.i_ProId << " born with strength" << " "
			<< ia_ProCos[cm.ia_ProOrd[cm.i_ProKey]] << "," << cm.ia_ProNum[cm.i_ProKey] << " " << sa_SmuNam[cm.ia_ProOrd[cm.i_ProKey]] << " in " <<cm.c_Nam << " headquarter" << endl;
	}
	return 0;
};
...
int main(){
	...
	for (int i_Case = 1; i_Case <i_Time+1 ; i_Case++){
		...
		{	
			...
			//if (e_Mblue.b_Flag){ e_W1.f_Run(e_Mblue); };
			
			if (e_Mred.b_Flag){ 
				cout << setw(3) << setfill('0') << e_W1.i_Hou << " ";
				e_Mred.f_ProSmu(e_W1); 
				//e_W1.f_Monitor(e_Mred); 
			};
			
			if (e_Mblue.b_Flag){
				cout << setw(3) << setfill('0') << e_W1.i_Hou << " ";
				e_Mblue.f_ProSmu(e_W1);
				//e_W1.f_Monitor(e_Mblue); 
			};
			e_W1.i_Hou++;
		}
	}
